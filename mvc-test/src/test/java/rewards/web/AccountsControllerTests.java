...
public class AccountsControllerTests {
	...
	public void indexFindsAllAccountsAndUsesListView() throws Exception {
		...
		assertEquals(AccountsController.VIEW_NAME_BASE + "list",
	...
	@Test
	public void showGetAnAccountAndUsesShowView() throws Exception {
		String accountNumber = "0";
		Account account = new Account(accountNumber, "accountName");
		when(rewardsApplicationService.findAccountAndCardsByNumber(accountNumber)) //
			.thenReturn(account);
		BindingAwareModelMap model = new BindingAwareModelMap();
		assertEquals(AccountsController.VIEW_NAME_BASE + "show",
				controller.show(accountNumber, model));
