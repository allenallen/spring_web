	...
	<div id="accountDetails">
		<ul>
			<li><fmt:message key="label.Account.number" />:
				${account.number} <spring:url var="editUrl" value="{number}/edit">
					<spring:param name="number" value="${account.number}" />
				</spring:url> <%-- TODO 08b: Show link based on role --%> <security:authorize
			...
			<li><fmt:message key="label.Account.receivingMonthlyEmailUpdate" />:
				<fmt:message key="label.${account.receivingMonthlyEmailUpdate}" /></li>
		</ul>
	</div>
	<div id="cards">
		<h2>
		...
		<c:if test="${!empty account.cards}">
			<table class="table">
				<thead>
					<tr>
						<th><fmt:message key="label.Account.card.number" /></th>
					</tr>
				</thead>
				<tbody>
